...
def generate():
    ...
    group = add(
        "group",
        isa="PBXGroup",
        children=source_refs + [plist, products],
        sourceTree="<group>",
    )
    sources = add(
        "sources",
    ...
    target_settings = {
        ...
        "PRODUCT_BUNDLE_IDENTIFIER": "app.gas.korea",
        "INFOPLIST_FILE": "Gas/Info.plist",
    ...
    target = add(
        "target",
        isa="PBXNativeTarget",
        buildConfigurationList=configs("target", target_settings),
    ...
    def encode(value, level=0):
        ...
        if isinstance(value, dict):
            return (
                "{\n"
                + "".join(
                    f"{pad}\t{json.dumps(str(k))} = {encode(v, level + 1)};\n"
